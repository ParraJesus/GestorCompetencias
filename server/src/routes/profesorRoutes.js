...
router.post("/create", (req, res) => {
    const { documento, tipoDocumento, nombre, apellido, usuario, contrasena, correo, tipo, titulo, estado } = req.body;

    db.query(
        "CALL Profesor_Crear(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [documento, tipoDocumento, nombre, apellido, usuario, contrasena, correo, tipo, titulo, estado],
        (err, result) => {
            if (err) {
                console.error(err);
