...
function EditarProfesor() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/evaluadores/evaluador/${id}`)
      .then((response) => {
        const data = response.data[0];
        setFormData({
          id: data.EVALUADOR_ID,
      ...
      .catch((error) => {
        console.error(error);
        alert("Error al cargar los datos del evaluador.");
      });
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/evaluadores`,
        formData
      );
      alert(response.data);
      navigate("/coordinador/evaluadores/");
    } catch (error) {
      console.error(error);
      alert("Hubo un error al actualizar el evaluador.");
    }
  };
